...
# Skill: Reversa Dyeus Bubble Graph & Diagnóstico de Saúde

## Propósito
Atuar como uma extensão analítica de alto nível (compatível com o framework **Reversa Dyeus**) para mapear a saúde arquitetural de um software. Por meio de análise estática e inspeção de dependências, o agente deve gerar um grafo visual (Bubble Graph) utilizando Mermaid.js e apresentar um diagnóstico técnico focado no acoplamento, modularidade e encapsulamento dos módulos do projeto, de forma retroativa (comparando versões) e progressiva (sugerindo próximos passos).
